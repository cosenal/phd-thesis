...
from a known set.
A scenario describing the problem is presented, first for the case where the 
...
prepares a quantum register $\reg{X}$ with the state $\rho_{k} \in \Pos(\X)$,
and sends it to Alice.
Alice's task it to identify the index $k$ by performing a measurement on the 
register $\reg{X}$.
...
The distinguishability of non-orthogonal states is a prolific topic on its own and 
